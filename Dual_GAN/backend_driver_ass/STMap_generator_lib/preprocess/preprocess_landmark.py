from backend_driver_ass.STMap_generator_lib.attachment.pytorch_face_landmark.common.utils import BBox
from backend_driver_ass.STMap_generator_lib.attachment.pytorch_face_landmark.models import MobileNet_GDConv
from backend_driver_ass.STMap_generator_lib.attachment.pytorch_face_landmark.models.mobilefacenet import MobileFaceNet
from backend_driver_ass.STMap_generator_lib.attachment.pytorch_face_landmark.models.pfld_compressed import PFLDInference
...
def pre_landmark():
    print('****************Preprocess_landmark INITIALISED**********************')
    #
    # global config
    #
    ...
    def landmark_face():
        ...
        for path in [video_path]:
            ...
            load_video_dir = load_dir
            output_video_dir =output_dir
            mkdir_if_missing(output_video_dir)
            print('load_video_dir_preprocess_detect = "{}"'.format(load_video_dir))
            print('output_video_dir_preprocess_landmark = "{}"'.format(output_video_dir))
            ...
            for b_idx, batch in enumerate(data_loader):
                ...
                for l_idx in range(batch_size):
                    ...
                    if new_bbox is None:
                        print('Continue:, start_idx = {}'.format(start_idx))
                        start_idx += 1
                        continue
                    else:
