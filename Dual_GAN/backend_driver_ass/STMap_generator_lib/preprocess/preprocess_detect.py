from backend_driver_ass.STMap_generator_lib.attachment.pytorch_face_landmark.Retinaface.layers.functions.prior_box import PriorBox
from backend_driver_ass.STMap_generator_lib.attachment.pytorch_face_landmark.Retinaface.utils.box_utils import decode
from backend_driver_ass.STMap_generator_lib.attachment.pytorch_face_landmark.Retinaface.utils.nms.py_cpu_nms import py_cpu_nms
from backend_driver_ass.STMap_generator_lib.attachment.pytorch_face_landmark.Retinaface.models.retinaface import RetinaFace
...
from backend_driver_ass.STMap_generator_lib.tools.net_tools import remove_prefix, check_keys
...
def pre_detect():
    ...
    print('****************Preprocess_detect INITIALISED**********************')
    MIN_SIZE = 54
    ...
    def load_model():
        device = "cuda"
        pretrained_path ="../attachment/pytorch_face_landmark/Retinaface/weights/mobilenet0.25_Final.pth"#'C://Users/sarth/Desktop/ML/Dual2/STMap_generator_lib/attachment/pytorch_face_landmark/Retinaface/weights/mobilenet0.25_Final.pth'
    ...
    def detect_face():
        ...
        for path in [video_path]:
            print(f'Path of the video file - {path}')
            output_preprocess_detect_video_dir = os.path.join(
                output_dir
            )
            mkdir_if_missing(output_preprocess_detect_video_dir)
            print('output_preprocess_detect_video_dir = "{}"'.format(output_preprocess_detect_video_dir))
            ...
            for idx, batch in enumerate(data_loader):
                ...
                for jdx in range(len(loc_b)):
                    ...
                    if len(det_bboxes) == 0:
                        print('Detect nothing: box = 0 : start_idx = {}'.format(start_idx))
                        start_idx += 1
                        continue
                    else:
                        ...
                        if rect.width() < MIN_SIZE or rect.height() < MIN_SIZE:
                            print('Detect nothing: too small : start_idx = {}'.format(start_idx))
                            start_idx += 1
                            continue
                        else:
                            output_video_frame_path = os.path.join(
                                output_preprocess_detect_video_dir,
                                str(start_idx)
                            )
