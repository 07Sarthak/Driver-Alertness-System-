...
class Normaliztion(object):
    ...
    def __call__(self, sample):
        video_x, video_y,  ecg_label, frame_rate, scale= sample['video_x'], sample['video_y'], sample['ecg'], sample['frame_rate'], sample['scale']
        new_video_x = (video_x - 127.5) / 128
        new_video_y = (video_y - 127.5) / 128
        return {'video_x': new_video_x, 'video_y': new_video_y, 'ecg': ecg_label, 'frame_rate': frame_rate, 'scale':scale}
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, sample):
        ...
        if p < 0.5:
            ...
            # print('no Flip')
            return {'video_x': video_x, 'video_y': video_y,'ecg': ecg_label, 'frame_rate': frame_rate, 'scale':scale}
...
class VIPL_train(Dataset):
    ...
    def __init__(self, scale,frames ,test=False, transform=None):
        ...
        # self.test = test
        self.folder="mtcnn_resized_collected_data"
        self.parent="/scratch/siddharths.scee.iitmandi/collected_data/"
        ...
        # subject_list=[i for i in subject_list if os.path.exists(self.parent+i+"/"+self.folder)]
    ...
    def __getitem__(self, idx):
        ...
        video_x = self.get_single_video_x(self.parent + self.vdPath_list[idx] + f'/{self.folder}/' , start_frame + 1)
        video_y = self.get_single_video_x(self.parent + self.vdPath_list[idx] + f'/{self.folder}/' , start_frame + 1)


        ecg_label = self.Trace[idx][start_frame:start_frame + self.frames]

        sample = {'video_x': video_x,'video_y': video_y, 'frame_rate': self.frame_rate[idx], 'ecg': ecg_label, 'scale':self.scale[self.idx_scale]}
    ...
    def get_single_video_x(self, video_jpgs_path, start_frame):
        ...
        for i in range(self.frames):
            ...
            video_x[i, :, :, :] = tmp_image
            # video_x.append(np.array(tmp_image))
