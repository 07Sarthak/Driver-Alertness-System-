...
class PURE_train(Dataset):
    ...
    def __init__(self, scale,frames ,test=False, transform=None):
        ...
        self.path_json = "/home/siddharths.scee.iitmandi/Home/DATASET-2"
        self.path_data = "/home/siddharths.scee.iitmandi/Home/DATASET-2"
        self.length = frames
        self.scale = scale        
        ...
        for i in range(0,len(self.videoList)):
            ...
            with open(tempPath + '/' + "ground_truth.txt", 'r') as f:
                # data = json.load(f)
                data=f.read()
                # data = data["/FullPackage"]
                ppg = [float(x) for x in f.readline().split()]
            segNum = len(ppg)//self.length
    ...
    def __getitem__(self, idx):
        for i in range(len(self.sampleGetIdList)):
            if(idx < self.sampleGetIdList[i]):
                ...
                break
        picPath =  self.path_data + self.videoList[vId] + '/frames/'
        startFrame = clipId *  self.length   
        video_x = self.get_single_video_x(picPath , startFrame)

        video_y = self.get_single_video_x(picPath , startFrame)
