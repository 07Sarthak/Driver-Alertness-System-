...
class PURE_train(Dataset):
    ...
    def __init__(self, scale,frames ,test=False, transform=None):
        ...
        self.path_json = "/scratch/siddharths.scee.iitmandi/PURE/"
        self.path_data = "/scratch/siddharths.scee.iitmandi/PURE/"
        self.length = frames
        self.scale = scale        
        ...
        for i in range(0,len(self.videoList)):
            ...
            segNum = int(len(os.listdir(tempPath + '/pic/1.0/')))//self.length
            # print(tempPath + '/pic/',len(os.listdir(tempPath + '/pic/')))
            # print(segNum*2*self.length)
